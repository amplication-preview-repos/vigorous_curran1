/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ForecastWhereUniqueInput } from "./ForecastWhereUniqueInput";
import { ValidateNested } from "class-validator";
import { Type } from "class-transformer";
import { ForecastUpdateInput } from "./ForecastUpdateInput";

@ArgsType()
class UpdateForecastArgs {
  @ApiProperty({
    required: true,
    type: () => ForecastWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ForecastWhereUniqueInput)
  @Field(() => ForecastWhereUniqueInput, { nullable: false })
  where!: ForecastWhereUniqueInput;

  @ApiProperty({
    required: true,
    type: () => ForecastUpdateInput,
  })
  @ValidateNested()
  @Type(() => ForecastUpdateInput)
  @Field(() => ForecastUpdateInput, { nullable: false })
  data!: ForecastUpdateInput;
}

export { UpdateForecastArgs as UpdateForecastArgs };
